...
<div class="collapse" id="forum" style="">
        ...
        <li class="nav-item menu-items active">
          ...
          <div class="collapse show" id="commande" style="">
            <ul class="nav flex-column sub-menu">
              <li class="nav-item active"> <a class="nav-link" href="listeCommande.php">Commande</a></li>
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
        ...
        <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title text-warning" align="center">Liste Commandes</h4>
                        <center>
                        <br/>
                        ...
                        <thead>
                            <tr >
                                <th class="text-info">Id_Commande</th>
                                <th class="text-info">Id_Client</th>
                                <th class="text-info">Date</th>
                                <th class="text-info">Status</th>
                                <th class="text-info">Prix Commande</th>
                                <th class="text-info">Methode payment</th>
                                <th class="text-info">Adresse</th>
                                <th class="text-info">Methode Livraison</th>
                                <th class="text-info">Update</th>
                                <th class="text-info">Distribuer</th>
                        ...
                        <tbody>
                            <?php foreach ($list as $p): ?>
                                <tr>
                                    <td class="text-light"><?php echo $p['id_commande']; ?></td>
                                    <td class="text-light"><?php  echo $p['id_client']; ?></td>
                                    <td class="text-light"><?php echo $p['date_commande']; ?></td>
                                    <td class="text-light"><?php echo $p['status']; ?></td>
                                    <td class="text-light"><?php echo $p['prix_commande']; ?></td>
                                    <td class="text-light"><?php echo $p['methode']; ?></td>
                                    <td class="text-light"><?php echo $p['ville'].' '. $p['code_postale'] .' '. $p['adresse']  ; ?></td>
                                    <td class="text-light"> <?php echo $p['livraison']; ?></td>
                                    <td>
                                        <form method="POST" action="updateCommande.php">
                                            ...
                                            <input type="hidden" value=<?PHP echo $p['id_commande']; ?> name="id_commande">
                                        </form>
                                    </td>

                                    <td>
                                        <form method="POST" action="updateCommande1.php"  onsubmit="return confirmDistribution('<?php echo $p['status']; ?>')">
                                            <input type="submit" name="update" class="btn btn-outline-primary btn-fw" value="DISTRIBUER">
                                            <input type="hidden" value=<?PHP echo $p['id_commande']; ?> name="id_commande" id='id_commande'>
                                        </form>
                                    </td>
                                    <td> <a href="deleteCommande.php?id_commande=<?php echo $p['id_commande']?>" class="btn btn-outline-danger btn-fw" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette commande ? ')"> DELETE </a> </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                        </table>
...
<script>
    function confirmDistribution(status) {
        if (status === 'NoConfirm') {
            return confirm("Cette commande n'est pas confirmée. Voulez-vous quand même la distribuer ?");
        }
        return true; // Sinon, la commande est déjà confirmée, pas besoin de confirmation supplémentaire
    }
</script>
</body></html>